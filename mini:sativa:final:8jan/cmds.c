...
Command *parse_pipeline(Token **tokens, envvar **env_list, shell_status *e_status)
{
    ...
    while (current_token) 
    {
        if (current_token->type == TOKEN_WORD || current_token->type == TOKEN_QUOTE || current_token->type == TOKEN_ENV_VAR || current_token->type == TOKEN_EXIT_STATUS) 
        {
            ...
            if (current_token->type == TOKEN_WORD && current_token->next && current_token->next->type == TOKEN_EXIT_STATUS && current_cmd->args != NULL)
            {
                char *cpy = strdup(current_token->value);
                current_token = current_token->next;
                current_token->value = ft_itoa(e_status->last_exit_status);
                char *join = ft_strjoin(cpy, current_token->value);
                current_token->type = TOKEN_WORD;
                current_token->value = strdup(join);
                // add_argument(current_cmd, current_token->value);
                // current_token = current_token->next->next;
            }
            else if (current_token->type == TOKEN_WORD && current_token->next && current_token->next->type == TOKEN_ENV_VAR && current_cmd->args != NULL)
            {
                char *cpy = strdup(current_token->value);
                current_token = current_token->next;
                char *env_value = get_env_var(env_list, current_token->value);
                if (env_value)
                    ...
                    current_token->value = strdup("NULL");
                char *join = ft_strjoin(cpy, current_token->value);
                current_token->type = TOKEN_WORD;
                current_token->value = strdup(join);
            }
            else if (current_token->type == TOKEN_EXIT_STATUS && current_token->next && current_token->next->type == TOKEN_WORD)
            {
                char *cpy = strdup(current_token->value);
                current_token = current_token->next;
                char *join = ft_strjoin(cpy, current_token->value);
