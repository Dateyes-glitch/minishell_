...
void execute_pipeline(Command *cmd, builtin_cmd_t *builtins, envvar **env_list, shell_status *e_status) 
{
    int pipe_fd[2];
...
int ft_execute_parsed_commands(Command *cmd, builtin_cmd_t *builtins, envvar **env_list, shell_status *e_status) 
{
    if (!cmd) return 1;
...
//    e_status.last_exit_status = 0;
    ...
    do 
    {
        ...
        status = ft_execute_parsed_commands(commands, builtins, &env_list, &e_status);
