...
void dup_to_stdo(Command *cmd, shell_status *e_status)
{
    char *com = "cat";
    char *executable_path = find_executable("cat", e_status);
    pid_t pid = fork();
    if (pid == 0) 
    { 
        char *envp[] = {NULL};
...
int execute_command_node(Command *cmd, builtin_cmd_t *builtins, envvar **env_list, unset_path_flag *unset_flag, shell_status *e_status) 
{
    if (ft_execute_builtin(cmd, builtins, env_list, unset_flag, e_status) == 0)
        return 0;
    else if (cmd->heredoc == 126)
    {
        handle_heredoc_to_fd(cmd);
        dup_to_stdo(cmd, e_status);
        return 0;
    }
    else
    {
...
void execute_pipeline(Command *cmd, builtin_cmd_t *builtins, envvar **env_list, shell_status *e_status, unset_path_flag *unset_flag) 
{
    ...
    while (current_cmd) 
    {
        ...
        if ((pid = fork()) == 0) 
        {
            ...
            close(pipe_fd[0]);
            execute_command_node(current_cmd, builtins, env_list, unset_flag, e_status);
            exit(EXIT_SUCCESS);
        }
        else if (pid > 0)
        {
...
int ft_execute_parsed_commands(Command *cmd, builtin_cmd_t *builtins, envvar **env_list, shell_status *e_status, unset_path_flag *unset_flag) 
{
    ...
    if (!cmd->next) 
        return execute_command_node(cmd, builtins, env_list, unset_flag, e_status); 
    else 
    {
