...
/*   Updated: 2025/01/22 20:50:47 by kforfoli         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	execute_fork(char *executable_path, char **args, char **envp, t_command *cmd)
{
	pid_t	pid;
...
int is_dir(t_command *cmd, char **args)
{
	char cwd[256];
    getcwd(cwd, sizeof(cwd));
    if (chdir(args[0]) == 0 && (args[0][0] == '.' && args[0][1] == '/'))
    {
        printf("%s : is a directory.\n", args[0]);
        cmd->exit_status = 126;
        chdir(cwd);
...
int	ft_execute_external(char **args, t_command *cmd, t_envvar **env_list)
{
	...
	if (args[0][0] == '.' && args[0][1] == '/')
		executable_path = ft_strdup(args[0]);
